...
const ELLIPSIS = "\u2026";
const ellipsisElement = <span key="more" className="more-ellipsis" title={`more${ELLIPSIS}`}>{ELLIPSIS}</span>
...
const escapeMap: {[k:number]: string} = {
  // Tab.
  9: "\\t",
  // Newline.
  0xa: "\\n",
 ...
 */
function interleave(items: string[], char: string) {
...
function rawCropString(text: string, limit?: number, alternativeText = ELLIPSIS) {
  ...
  if (text.length > limit) {
    return (
      text.substring(0, Math.ceil(halfLimit)) +
      alternativeText +
      text.substring(text.length - Math.floor(halfLimit))
    );
  }
...
function cropString(text: string, limit?: number, alternativeText?: string) {
 ...
 */
function containsURL(grip: string) {
 ...
 */
function escapeString(str: string, escapeWhitespace: boolean = false) {
  let quote = '"';
  ...
  return `${quote}${str.replace(regexp, (match, offset) => {
    ...
    return `\\u${`0000${c.toString(16)}`.substring(-4)}`;
  })}${quote}`;
}
...
function escapeNewLines(value: string) {
  return value.replace(/\r/gm, "\\r").replace(/\n/gm, "\\n");
}
function cropMultipleLines(text: string, limit: number) {
  return escapeNewLines(cropString(text, limit));
}

function parseURLEncodedText(text: URLSearchParams): {
  name: string,
  value: string,
}[] {
  const params: {
...
function parseURLParams(url: string) {
  ...
  return parseURLEncodedText(_url.searchParams);
}
...
function splitURLBase(url: string) {
  if (!isDataURL(url)) {
...
function isDataURL(url: string) {
  return url && url.substring(0, 5) == "data:";
}

function splitURLTrue(url: string) {
...
function getURLDisplayString(url: string) {
 ...
 */
function maybeEscapePropertyName(name: string) {
...
function getGripPreviewItems(grip: {
  preview: {
    ...
    ownProperties?: {[k: string]: any};
    safeGetterValues?: {[k: string]: any};
  };
  ...
  if (grip.preview && grip.preview.entries) {
    return grip.preview.entries.reduce((res: any[], entry: any) => res.concat(entry), []);
  }
 ...
 */
function getGripType(object: any, noGrip: boolean = false) {
 ...
 */
function cleanupStyle(userProvidedStyle: string, createElement: (tag: string) => HTMLElement) {
  ...
  const dummy = createElement("div") as any;
  ...
  return Array.from(dummy.style as string[])
    .filter(name => {
      ...
      // a data URL.
      return Array.from((dummy.style[name] as string).matchAll(cssUrlRegex))
        .map(match => match[1])
